  ...
  const signInWithTwitter = async () => {
    try {
      ...
      const { data, error } = await supabase.auth.signInWithOAuth({
        provider: "twitter",
        options: {
          redirectTo: `${window.location.origin}/claim-og-nft`,
        },
      });
        ...
        <div className="relative z-10 container mx-auto px-4 py-12 max-w-7xl">
          ...
          <div className="flex flex-col lg:flex-row items-center justify-center gap-8 lg:gap-12">
            ...
            <div className="w-full lg:w-auto lg:flex-1 max-w-2xl order-2">
              {/* Error Display */}
              {error && (
                <div className="mb-6 p-4 border-2 border-yello-500/30 rounded-lg flex items-start gap-3">
                  <X
                    className="text-yellow-500/80 flex-shrink-0 mt-0.5"
                    size={20}
                  />
                  <p className="text-yellow-400/80">{error}</p>
                </div>
              )}
                ...
                {currentStep === "eligibility" && (
                  <div className="text-center space-y-6">
                    {isCheckingEligibility ? (
                      ...
                      <>
                        <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto">
                          <X className="text-yellow-500/80" size={40} />
                        </div>
                        <h2 className="text-2xl font-bold text-yellow-500/80">
                          Not Eligible
                        </h2>
                          ...
                          {twitterHandle && (
                            <div className="mt-4 p-4 rounded-lg bg-gray-800/30 border border-gray-700/30">
                              ...
                              <div className="flex flex-col gap-2">
                                <button
                                  onClick={disconnectTwitter}
                                  className="text-xs text-gray-400/70 hover:text-yellow-400/70 underline transition-colors text-center"
                                >
